...
const ExploreItem: React.FC<ExploreItemProps> = ({
  ...
  return (
    <div
      className={clsx(
        `relative  ${item?.type === "creator"  ? "h-80 w-full" :"h-40 w-40"} bg-cover bg-center rounded-lg overflow-hidden group transform transition-transform duration-300 hover:scale-105`,
        containerClassName
      )}
      ...
      <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end items-center">
        <div className="text-center text-white px-4 mb-12">
          ...
          <p className="text-sm mb-2">
            {item?.type === "creator" ? `Description : ${item?.description}`: `Symbol : ${item?.symbol}`}
          </p>
          <p className="text-sm font-semibold">
            {item?.type === "creator" ? `${item?.followers} followers` : `${item?.marketCap} marketCap` } 
          </p>
        </div>
        ...
        <Button
          ...
          href={item?.type === "creator" ? `/explore/${item.id}` : `/coins/${item.id}`}
        >
          Bet on {item?.type === "creator" ? "Creator" : "Coin"}
        </Button>
      </div>
    </div>
  );
